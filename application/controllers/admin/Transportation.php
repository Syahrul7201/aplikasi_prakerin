...
class Transportation extends CI_Controller{
	...
	public function p($action,$id=''){
		if($action=='insert'){
			...
			foreach($required as $f){
				if(empty($_POST[$f])){
					$this->session->set_flashdata('pesan','<div class="alert alert-danger">Masih ada data yang kosong</div>');
    ...
    public function truncate(){
		...
		$this->session->set_flashdata('pesan','<div class="alert alert-success">Data berhasil dikosongkan</div>');
		redirect('admin/transportation');
    }
}